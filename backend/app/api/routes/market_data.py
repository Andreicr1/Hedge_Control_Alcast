    ...
    for symbol in symbols:
        ...
        try:
            ...
            audit_event(
                ...
                current_user.id,
                {"symbol": symbol, "price": price, "currency": currency, "fx": mp.fx, "source": source_name},
            )
...
def fetch_from_yahoo(
    ...
    pricing_source: Optional[str] = Query(None, description="Optional pricing source label to store"),
    fx_only: bool = Query(False, description="If true, only fetch FX symbols (=X) and flag as fx"),
    db: Session = Depends(get_db),
    current_user: models.User = Depends(require_roles(models.RoleName.admin, models.RoleName.financeiro)),
):
    return _ingest_from_yahoo(symbols, db, current_user, pricing_source=pricing_source, fx_only=fx_only)
...
def fetch_fx_from_yahoo(
    ...
    return _ingest_from_yahoo(symbols, db, current_user, pricing_source=pricing_source, fx_only=True)
