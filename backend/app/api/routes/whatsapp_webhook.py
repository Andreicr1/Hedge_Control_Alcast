...
def _parse_message(text: str) -> dict:
  ...
  validity_match = re.search(r"validade[:\s]*([0-9]{2}[\/\-][0-9]{2}[\/\-][0-9]{2,4}|[0-9]{4}\-[0-9]{2}\-[0-9]{2})", text, re.IGNORECASE)
  ...
  observations = re.sub(r"validade[:\s]*([0-9]{2}[\/\-][0-9]{2}[\/\-][0-9]{2,4}|[0-9]{4}\-[0-9]{2}\-[0-9]{2})", "", observations, flags=re.IGNORECASE)
...
async def whatsapp_webhook(
  ...
  if not counterparty:
    raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Contraparte não identificada")
  ...
  invitation = (
      ...
      .filter(models.RfqInvitation.rfq_id == rfq.id, models.RfqInvitation.counterparty_id == counterparty.id)
      .first()
  )
  if not invitation:
