...
def whatsapp_webhook(
    ...
    db.commit()
    db.refresh(message)
    logger.info("whatsapp.inbound", extra={"message_id": message.message_id, "phone": message.phone})
    return message
        ...
        rows.append(
            ",".join(
                [
                    ...
                    m.direction.value,
                    m.status.value,
                    f'"{(m.phone or "").replace('"', '""')}"',
                    f'"{(m.message_id or "").replace('"', '""')}"',
                    f'"{(m.content_text or "").replace('"', '""')}"',
