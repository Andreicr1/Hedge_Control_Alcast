...
from app.schemas import RfqReportItem, RfqAttemptReport, RfqExportItem
...
def rfq_by_counterparty(
    ...
    if status:
        query = query.join(models.Rfq, models.RfqQuote.rfq_id == models.Rfq.id).filter(models.Rfq.status == status)
    if start_date:
...
def rfq_attempts(
    ...
    query = db.query(models.RfqSendAttempt).join(models.Rfq, models.RfqSendAttempt.rfq_id == models.Rfq.id)
    if status:
...
def rfq_export(
    ...
    for rfq in rfqs:
        ...
        for provider in providers:
            ...
            attempt = sorted(attempts_for_provider, key=lambda a: a.created_at, reverse=True)[0] if attempts_for_provider else None
            quote = sorted(quotes_for_provider, key=lambda q: q.created_at, reverse=True)[0] if quotes_for_provider else None
            ...
            row_time = quote.created_at if quote else (attempt.created_at if attempt else rfq.created_at)
            ...
            rows.append(
                RfqExportItem(
                    rfq_id=rfq.id,
                    rfq_status=rfq.status.value if hasattr(rfq.status, "value") else str(rfq.status),
