...
from app.schemas import MtmRecordCreate, MtmRecordRead, MtmComputeRequest, MtmComputeResponse
...
def create_mtm_record(
    ...
    audit_event(
        "mtm.created",
        current_user.id,
        {"mtm_id": record.id, "object_type": record.object_type.value, "object_id": record.object_id},
    )
    return record
