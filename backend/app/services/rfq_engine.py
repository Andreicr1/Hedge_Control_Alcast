...
def generate_rfq_text(
    ...
    if l2_adj is not None:
        if l1_adj.price_type == PriceType.FIX and (l1_adj.order and l1_adj.order.order_type == OrderType.RESTING) and (l1_adj.fixing_date is None) and l2_adj.price_type == PriceType.AVG:
            ppt1 = compute_ppt_for_leg(l1_adj, cal)
            if ppt1:
                leg2_text = (leg2_text or "") + f", ppt {fmt_date_short(ppt1)}"
        if l2_adj.price_type == PriceType.FIX and (l2_adj.order and l2_adj.order.order_type == OrderType.RESTING) and (l2_adj.fixing_date is None) and l1_adj.price_type == PriceType.AVG:
            ppt2 = compute_ppt_for_leg(l2_adj, cal)
            if ppt2:
