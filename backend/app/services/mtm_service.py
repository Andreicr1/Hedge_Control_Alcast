...
def _latest_market_price(db: Session, symbol: str, source: Optional[str] = None, fx_only: bool = False):
...
def _latest_fx_rate(db: Session, fx_symbol: Optional[str], source: Optional[str] = None) -> Optional[float]:
    if not fx_symbol:
        return None
...
def _apply_price_adjustments(price: float, haircut_pct: Optional[float], price_shift: Optional[float]) -> float:
    adjusted = price
