...
def _compute_for_exposure(db: Session, exposure_id: int, price: float) -> tuple[float, float, str | None, str | None]:
...
def _compute_for_hedge(db: Session, hedge_id: int, price: float) -> tuple[float, float, str | None, str | None]:
...
def _compute_for_net(db: Session, product: Optional[str], period: Optional[str], price: float) -> tuple[float, float, str | None]:
...
def create_snapshot(db: Session, payload: MTMSnapshotCreate) -> models.MTMSnapshot:
    ...
    if payload.object_type in {models.MarketObjectType.exposure, models.MarketObjectType.hedge} and payload.object_id is None:
    ...
    if payload.object_type == models.MarketObjectType.exposure:
        qty, mtm_val, prod, derived_period = _compute_for_exposure(db, payload.object_id or 0, payload.price)
        period = payload.period or derived_period
    elif payload.object_type == models.MarketObjectType.hedge:
        qty, mtm_val, prod, hedge_period = _compute_for_hedge(db, payload.object_id or 0, payload.price)
...
def list_snapshots(
    ...
    q = q.order_by(models.MTMSnapshot.created_at.desc())
    if latest:
        snap = q.first()
