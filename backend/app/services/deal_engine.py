...
def link_purchase_order_to_deal(db: Session, po: models.PurchaseOrder, deal_id: Optional[int]) -> models.Deal:
    if not deal_id:
...
def link_hedge_to_deal(db: Session, hedge: models.Hedge, deal_id: Optional[int]) -> models.Deal:
    ...
    direction = models.DealDirection.buy if getattr(hedge, "is_buy", True) else models.DealDirection.sell
    link = models.DealLink(
        deal_id=deal.id,
...
def calculate_deal_pnl(db: Session, deal_id: int, persist_snapshot: bool = True) -> Optional[Dict]:
    ...
    links: List[models.DealLink] = db.query(models.DealLink).filter(models.DealLink.deal_id == deal_id).all()
    ...
    hedge_ids = [link.entity_id for link in links if link.entity_type == models.DealEntityType.hedge]
            ...
            physical_legs.append(
                {
                    ...
                    "pricing_reference": so.pricing_period or (so.fixing_deadline.isoformat() if so.fixing_deadline else None),
            ...
            physical_legs.append(
                {
                    ...
                    "pricing_reference": po.pricing_period or (po.fixing_deadline.isoformat() if po.fixing_deadline else None),
    ...
    if hedge_ids:
        for hedge in db.query(models.Hedge).filter(models.Hedge.id.in_(hedge_ids)).all():
            ...
            if hedge.mtm_value is not None:
                ...
                mtm_price = hedge.current_market_price if hedge.current_market_price is not None else entry_price
            else:
