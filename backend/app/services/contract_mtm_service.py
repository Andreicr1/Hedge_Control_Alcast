...
def _latest_cash_publish_date(db: Session) -> Optional[date]:
    """
    Returns the latest day we have a Cash observation.
    Priority is the authoritative settlement series; proxies are fallback only.
    """
    q = (
        db.query(models.MarketPrice.as_of)
        .filter(
            models.MarketPrice.symbol.in_(
                [AL_CASH_SETTLEMENT_SYMBOL, AL_CASH_MID_SYMBOL, AL_CASH_BID_SYMBOL, AL_CASH_ASK_SYMBOL]
            )
        )
...
def _cash_price_by_day(
    ...
    """
    Build a daily Cash price series.

    Priority per day:
    1) ALUMINUM_CASH_SETTLEMENT (Westmetall official Cash-Settlement)  [authoritative]
    2) ALUMINUM_CASH_MID (proxy)
    3) (ALUMINUM_CASH_BID + ALUMINUM_CASH_ASK)/2 (proxy)
    4) BID or ASK alone (proxy)
    """
    ...
    # Keep last obs per day/symbol
    latest: dict[tuple[date, str], models.MarketPrice] = {}
    for r in rows:
        d = r.as_of.date()
        key = (d, r.symbol)
        latest[key] = r
    ...
    day = start
    while day <= end:
        settlement = latest.get((day, AL_CASH_SETTLEMENT_SYMBOL))
        if settlement:
            out[day] = float(settlement.price)
            day = day + timedelta(days=1)
            continue
        mid = latest.get((day, AL_CASH_MID_SYMBOL))
        if mid:
            out[day] = float(mid.price)
            day = day + timedelta(days=1)
            continue
        bid = latest.get((day, AL_CASH_BID_SYMBOL))
        ask = latest.get((day, AL_CASH_ASK_SYMBOL))
        if bid and ask:
            out[day] = float((bid.price + ask.price) / 2.0)
        elif bid:
            out[day] = float(bid.price)
        elif ask:
            out[day] = float(ask.price)
        day = day + timedelta(days=1)
    return out
...
def _extract_avg_window_from_trade_specs(spec: dict[str, Any]) -> tuple[Optional[date], Optional[date]]:
    """
...
def _extract_fixed_price_and_side(trade_snapshot: dict[str, Any]) -> tuple[Optional[float], Optional[str]]:
...
def _extract_quantity_mt(trade_snapshot: dict[str, Any], rfq: Optional[models.Rfq]) -> float:
    legs = (trade_snapshot or {}).get("legs") or []
    for leg in legs:
...
def compute_mtm_for_contract_avg(
    ...
    realized_avg, end_used, last_published = compute_realized_avg_cash(db, obs_start, obs_end, as_of_date=as_of)
