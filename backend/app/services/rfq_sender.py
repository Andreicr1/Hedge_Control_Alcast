...
class SendResult:
    def __init__(self, status: SendStatus, provider_message_id: Optional[str] = None, error: Optional[str] = None):
        self.status = status
...
def send_rfq_message(
    ...
    *,
    ...
    return last_result or SendResult(status=SendStatus.failed, provider_message_id=provider_id, error="unknown error")
...
def _send_email(message: str, meta: Dict[str, Any], provider_id: str) -> SendResult:
    ...
    if meta.get("force_failure") or meta.get("_force_failure_override"):
        return SendResult(status=SendStatus.failed, provider_message_id=provider_id, error="email send failed")
...
def _send_api(message: str, meta: Dict[str, Any], provider_id: str) -> SendResult:
    ...
    if meta.get("force_failure") or meta.get("_force_failure_override"):
        return SendResult(status=SendStatus.failed, provider_message_id=provider_id, error="api send failed")
    status = SendStatus.queued if meta.get("async_mode") else SendStatus.sent
    return SendResult(status=status, provider_message_id=provider_id)
...
def _send_whatsapp(message: str, meta: Dict[str, Any], provider_id: str) -> SendResult:
    ...
    if meta.get("force_failure") or meta.get("_force_failure_override"):
        return SendResult(status=SendStatus.failed, provider_message_id=provider_id, error="whatsapp send failed")
    status = SendStatus.queued if meta.get("async_mode") else SendStatus.sent
    return SendResult(status=status, provider_message_id=provider_id)
