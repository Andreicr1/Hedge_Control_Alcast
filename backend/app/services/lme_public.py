...
LME_ALU_TRADING_SUMMARY_URL = "https://www.lme.com/en/metals/non-ferrous/lme-aluminium#Trading+summary"
...
async def fetch_lme_aluminum_intraday_snapshot(headless: bool = True) -> LmeAluminumIntradaySnapshot:
    """
    ...
    async with async_playwright() as p:
        ...
        date_text = await page.locator("text=Intraday prices").locator("..").locator("text=/\\d{2} \\w{3} \\d{4}/").first.inner_text()
        as_of_date = _parse_lme_date(date_text)

        # "Prices in US$"
        currency_text = await page.locator("text=Prices in").first.inner_text()
        currency = "USD" if "US$" in currency_text else currency_text.replace("Prices in", "").strip()
        ...
        for i in range(await quotes_rows.count()):
            ...
            cells = [await row.locator("th,td").nth(j).inner_text() for j in range(await row.locator("th,td").count())]
            if len(cells) < 5:
                continue
            quotes.append(
                LmeIntradayQuoteRow(
...
async def fetch_lme_aluminum_dashboard_prices(headless: bool = True) -> LmeAluminumDashboardPrices:
    ...
    async with async_playwright() as p:
        ...
        if three_month_last is None:
            raise RuntimeError("Não consegui extrair 3-month last traded da tabela Intraday (Last traded prices)")
